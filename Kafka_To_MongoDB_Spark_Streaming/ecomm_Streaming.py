...
payments_df = spark \
    ...
    .option("kafka.security.protocol", "SASL_SSL") \
    .option("kafka.sasl.mechanism", "PLAIN") \
    .option("kafka.sasl.jaas.config", "org.apache.kafka.common.security.plain.PlainLoginModule required username='7TUSIFBKFOX4UR' password='Ks4s1dZAgPEtSwRhTwxv5MprrvTd5oaLaZ6en79iYAgBTeoeZ8tJw';") \
    .load() \
    .selectExpr("CAST(value AS STRING) as value") \
...
query = joined_df.writeStream \
    ...
    .option('spark.mongodb.connection.uri', "mongodb+srv://chkul:<password>@mongo-db-cluster-new.abc123.mongodb.net/?retryWrites=true&w=majority&appName=mongo-db-cluster-new") \
